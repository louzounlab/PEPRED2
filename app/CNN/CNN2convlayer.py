...
from CNN.PLSuperModel import SuperModel
...
class CNN(SuperModel):
    ...
    def forward_to_last_layer(self, x, b=None):
        x = x.type(torch.float32)
        x = torch.unsqueeze(x, 1)
        x = self.cnn(x)
        if self.use_max_pool:
        ...
        if b is not None:
            x = torch.cat([x, b], dim=1).type(torch.float32)
        cut_lin = nn.Sequential(*list(self.lin.children())[:5])
        return cut_lin(x).type(torch.float32)
