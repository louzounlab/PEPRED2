...
def get_data_train_test(train_x, train_y, test_x, test_y, path_of_2D_matrix):
    org_path = os.getcwd()
...
def after_split_from_pd(otu_path, index_path_train, index_path_test):
    ...
    otu_test = pd.read_csv(index_path_test, index_col=0).index
    otu = pd.read_csv(otu_path, index_col=0)
    otu_train = otu.loc[otu_train]
    otu_test = otu.loc[otu_test]
    return otu_train, otu_test

def before_split_from_pd(otu_path, size):
    otu_train = pd.read_csv(otu_path, index_col=0).iloc[:int(0.8*size),:].index
    otu_test = pd.read_csv(otu_path, index_col=0).iloc[int(0.8*size):,:].index
    otu = pd.read_csv(otu_path, index_col=0)
    otu_train = otu.loc[otu_train]
    otu_test = otu.loc[otu_test]
    return otu_train, otu_test
...
def load_nni_data(name_of_dataset, D_mode, dendogram=False, tag_name=None, with_map=False, after_split=False, target_bacteria=None):
    """
    ...
    if name_of_dataset == "nugent":
        ...
        if D_mode == "IEEE":
            ...
            if after_split == False:
                otu = pd.read_csv(
                    "Data/Nugent_vagina/nugrelative_mean_rare_bact5.csv",
                    index_col=0)
            else:
                otu_train, otu_test = after_split_from_pd(
    ...
    # for roy
    #####################################################
    if name_of_dataset == "vaginal_swaps":
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv("../../swaps/gMic/split_datasets/Vaginal_swaps_split_dataset/merged_train_tags.csv", index_col=0)
            tag_test = pd.read_csv("../../swaps/gMic/split_datasets/Vaginal_swaps_split_dataset/merged_test_tags.csv", index_col=0)
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            if after_split == False:
                otu = pd.read_csv(
                    "2D_otus_dendogram_ordered/0_fixed_ordered_n_all_otu_sub_pca_log_tax_7.csv",
                    index_col=0)
            else:
                otu_train, otu_test = after_split_from_pd(
                    "2D_otus_dendogram_ordered/0_fixed_ordered_n_all_otu_sub_pca_log_tax_7.csv",
                    "../gMic/split_datasets/Vaginal_swaps_split_dataset/merged_train_microbiome.csv",
                    "../gMic/split_datasets/Vaginal_swaps_split_dataset/merged_test_microbiome.csv"
                )

    ################################################
    # saliva gdm in time
    ################################################
    if name_of_dataset == "saliva_gdm":
        size = pd.read_csv('../../data_microbiome_in_time/saliva/saliva_ys.csv').shape[0]
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv('../../data_microbiome_in_time/saliva/bacteria_tag_files/' + target_bacteria +'.csv',
                                    index_col=0).iloc[:int(0.8*size),:]
            tag_test = pd.read_csv('../../data_microbiome_in_time/saliva/bacteria_tag_files/' + target_bacteria +'.csv',
                                   index_col=0).iloc[int(0.8*size):,:]
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            path_of_2D_matrix = "../../data_microbiome_in_time/saliva/images"
            if after_split == False:
                otu = pd.read_csv("",index_col=0)
            else:
                otu_train, otu_test = before_split_from_pd(
                    "../../data_microbiome_in_time/saliva/saliva_full_data.csv",size)

    if name_of_dataset == "gdm_dataset":
        size = pd.read_csv('../../data_microbiome_in_time/gdm/gdm_ys.csv').shape[0]
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv('../../data_microbiome_in_time/gdm/bacteria_tag_files/' + target_bacteria +'.csv',
                                    index_col=0).iloc[:int(0.8*size),:]
            tag_test = pd.read_csv('../../data_microbiome_in_time/gdm/bacteria_tag_files/' + target_bacteria +'.csv',
                                   index_col=0).iloc[int(0.8*size):,:]
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            path_of_2D_matrix = "../../data_microbiome_in_time/gdm/images"
            if after_split == False:
                otu = pd.read_csv(
                    "",
                    index_col=0)
                ...
                otu_train, otu_test = before_split_from_pd(
                    "../../data_microbiome_in_time/gdm/OTU_merged_General_task.csv",size)
    if name_of_dataset == "vitamine_A":
        size = pd.read_csv('../../data_microbiome_in_time/vitamineA/VitamineA_ys.csv').shape[0]
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv('../../data_microbiome_in_time/vitamineA/bacteria_tag_files/' + target_bacteria +'.csv',
                                    index_col=0).iloc[:int(0.8*size),:]
            tag_test = pd.read_csv('../../data_microbiome_in_time/vitamineA/bacteria_tag_files/' + target_bacteria +'.csv',
                                   index_col=0).iloc[int(0.8*size):,:]
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            path_of_2D_matrix = "../../data_microbiome_in_time/vitamineA/images"
            if after_split == False:
                otu = pd.read_csv( "",                    index_col=0)
            else:
                otu_train, otu_test = before_split_from_pd(
                    "../../data_microbiome_in_time/vitamineA/OTU_merged_General_task.csv",size)

    if name_of_dataset == "mucositis":
        size = pd.read_csv('../../data_microbiome_in_time/mucositis/mucositis_ys.csv').shape[0]
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv('../../data_microbiome_in_time/mucositis/bacteria_tag_files/' + target_bacteria +'.csv',
                                    index_col=0).iloc[:int(0.8*size),:]
            tag_test = pd.read_csv('../../data_microbiome_in_time/mucositis/bacteria_tag_files/' + target_bacteria +'.csv',
                                   index_col=0).iloc[int(0.8*size):,:]
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            path_of_2D_matrix = "../../data_microbiome_in_time/mucositis/images"
            if after_split == False:
                otu = pd.read_csv(
                    "",                    index_col=0)
            else:
                otu_train, otu_test = before_split_from_pd(
                    "../../data_microbiome_in_time/mucositis/OTU_merged_General_task.csv",size)
    if name_of_dataset == "allergy_time_predictions":
        size = pd.read_csv('../../data_microbiome_in_time/allergy/Allergy_ys.csv').shape[0]
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv('../../data_microbiome_in_time/allergy_time_predictions/bacteria_tag_files/' + target_bacteria +'.csv',
                                    index_col=0).iloc[:int(0.8*size),:]
            tag_test = pd.read_csv('../../data_microbiome_in_time/allergy_time_predictions/bacteria_tag_files/' + target_bacteria +'.csv',
                                   index_col=0).iloc[int(0.8*size):,:]
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            path_of_2D_matrix = "../../data_microbiome_in_time/allergy/images"
            if after_split == False:
                otu = pd.read_csv(
                    "",                    index_col=0)
            else:
                otu_train, otu_test = before_split_from_pd(
                    "../../data_microbiome_in_time/allergy/OTU_merged_General_task_for_reg.csv",size)

    if name_of_dataset == "GVHD":
        size = pd.read_csv('../../data_microbiome_in_time/GVHD/ys.csv').shape[0]
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv('../../data_microbiome_in_time/GVHD/bacteria_tag_files/' + target_bacteria +'.csv',
                                    index_col=0).iloc[:int(0.8*size),:]
            tag_test = pd.read_csv('../../data_microbiome_in_time/GVHD/bacteria_tag_files/' + target_bacteria +'.csv',
                                   index_col=0).iloc[int(0.8*size):,:]
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            path_of_2D_matrix = "../../data_microbiome_in_time/GVHD/images"
            if after_split == False:
                otu = pd.read_csv(
                    "",                    index_col=0)
            else:
                otu_train, otu_test = before_split_from_pd(
                    "../../data_microbiome_in_time/GVHD/OTU_merged_General_task.csv",size)

    if name_of_dataset == "pnas":
        size = pd.read_csv('../../data_microbiome_in_time/pnas/ys.csv').shape[0]
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv('../../data_microbiome_in_time/pnas/bacteria_tag_files/' + target_bacteria +'.csv',
                                    index_col=0).iloc[:int(0.8*size),:]
            tag_test = pd.read_csv('../../data_microbiome_in_time/pnas/bacteria_tag_files/' + target_bacteria +'.csv',
                                   index_col=0).iloc[int(0.8*size):,:]
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            path_of_2D_matrix = "../../data_microbiome_in_time/pnas/images"
            if after_split == False:
                otu = pd.read_csv(
                    "",                    index_col=0)
            else:
                otu_train, otu_test = before_split_from_pd(
                    "../../data_microbiome_in_time/pnas/OTU_merged_General_task.csv",size)
    if name_of_dataset == "diab":
        size = pd.read_csv('../../data_microbiome_in_time/diab/ys.csv').shape[0]
        if after_split == False:
            pass
        else:
            tag_train = pd.read_csv('../../data_microbiome_in_time/diab/bacteria_tag_files/' + target_bacteria +'.csv',
                                    index_col=0).iloc[:int(0.8*size),:]
            tag_test = pd.read_csv('../../data_microbiome_in_time/diab/bacteria_tag_files/' + target_bacteria +'.csv',
                                   index_col=0).iloc[int(0.8*size):,:]
            group = tag_train["Group"]
        ...
        if D_mode == "IEEE":
            ...
            path_of_2D_matrix = "../../data_microbiome_in_time/diab/images"
            if after_split == False:
                otu = pd.read_csv(
                    "",                    index_col=0)
            else:
                otu_train, otu_test = before_split_from_pd(
                    "../../data_microbiome_in_time/diab/OTU_merged_General_task.csv",size)
