...
from CNN.textreeCreate import create_tax_tree
...
def dendogram_ordering(otu, df, save=False, N=None):
    ...
    if save is not False:
        ...
        # df.to_csv(
        #     "2D_otus_dendogram_ordered/Stool_israel_GDM_Adi/0_fixed_ordered_n_all_otu_sub_pca_log_tax_7")

        # df.to_csv(
        #     "2D_otus_dendogram_ordered/wb_fs/0_fixed_ordered_n_all_otu_sub_pca_log_tax_7.csv")
    ...
    if save is not False:
        ...
        for m, index in zip(otu, df.index):
            ...
            #m.dump(f"2D_otus_dendogram_ordered/PNAS/{index}.npy")
...
def main(otu,save):
    df = pd.read_csv(otu, index_col=0)
    otus2d, names = otu22d(df, False, with_names=True)
    dendogram_ordering(otus2d, df, save=save, N=names)

if __name__ == '__main__':
    # df = pd.read_csv("Data/BGU_otu_sum_relative_rare_bact_5_tax_7.csv", index_col=0)  # BGU
    # df = pd.read_csv("Data/Allergy/Milk_no_milk_no_rep_T0_T1_controls/otus.csv", index_col=0) # allergy_time_predictions milk
    ...
    #df = pd.read_csv("data/FOR_ROY/merged_full_microbiome_no_extra_features.csv", index_col=0)



    # df = pd.read_csv("../../data_microbiome_in_time/pnas/OTU_merged_General_task.csv", index_col=0)
    #
    # otus2d, names = otu22d(df, False, with_names=True)  # df.iloc[:10]
    # dendogram_ordering(otus2d, df, save="../../data_microbiome_in_time/pnas/images",
    #                    N=names)  # , save="2D_otus_dendogram_ordered/PNAS")

    main("../gMic/split_datasets/IBD_split_dataset/OTU_IBD_after_mipmlp_Genus.csv", "../gMic/split_datasets/IBD_split_dataset/images")