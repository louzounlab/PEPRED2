...
from naeive_model import Naeive
# from captum.attr import IntegratedGradients
from CNN1convlayer import CNN_1l
...
def POC(train_dataset, valid_dataset, test_dataset, y_train, y_valid, y_test, augmented_dataset,
    ...
    if task == "class":
        ...
        return acc_tr, acc_val, auc_tr, auc_val, f1_tr_micro, f1_val_micro, f1_tr_macro, f1_val_macro, acc, auc, f1_micro, f1_macro, cm, model
...
def main():
    ...
    while e < 10:
        try:
            ...
            try:
                r2_tr, r2_te, c_tr, c_te, f1_tr_mi, f1_te_mi, f1_tr_ma, f1_te_ma, r2, c, f1_mi, f1_ma, cm, model2 = POC(*V1,
                                                                                                                model=model,
                                                                                                                parms=D,
                                                                                                                mode=biomarkers,
                                                                                                                task=task,
                                                                                                                weighted=p.w)
                train = pd.read_csv('../gMic/split_datasets/Vaginal_swaps_split_dataset/merged_train_tags.csv')
                test =  pd.read_csv('../gMic/split_datasets/Vaginal_swaps_split_dataset/merged_test_tags.csv')
                dict = {}
                for sample in train['ID']:
                    image = torch.tensor(np.load(('2D_otus_dendogram_ordered/' + str(sample) + '.npy'), allow_pickle=True))
                    image = image.reshape(1, 7, -1)
                    dict[sample] = model2.forward_to_last_layer(image).detach().numpy().tolist()
                    dict[sample] = [item for sublist in dict[sample] for item in sublist]
