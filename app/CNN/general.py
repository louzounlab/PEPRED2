...
def apply_tsne(data, n_components=15, visualize=False):
    ...
    str_to_print = ''
    return pd.DataFrame(data_components).set_index(data.index
        ), tsne, str_to_print
...
def apply_pca(data, n_components=15, visualize=False):
    ...
    str_to_print = str('Explained variance per component: \n' + '\n'.join([
        ('Component ' + str(i) + ': ' + str(component) +
        ', Accumalative variance: ' + str(accu_var)) for accu_var, (i,
        component) in zip(pca.explained_variance_ratio_.cumsum(), enumerate
    ...
    if visualize:
        ...
        plt.bar(np.arange(0, n_components), height=pca.
            explained_variance_ratio_)
        plt.title(
            f'PCA - Explained variance using {n_components} components: {pca.explained_variance_ratio_.sum()}'
            )
        plt.xlabel('PCA #')
        plt.xticks(list(range(0, n_components)), list(range(1, n_components +
            1)))
...
def draw_horizontal_bar_chart(data, names=None, title=None, ylabel=None,
    xlabel=None, use_pos_neg_colors=True, left_padding=0.4):
    fig, ax = plt.subplots()
...
def convert_pca_back_orig(pca_components, w, original_names=None, visualize
    =False, title='Bacteria Coeff', ylabel='Bacteria', xlabel='Coeff Value'):
    coeff = np.dot(w, pca_components)
    if original_names is None:
        object_to_return = pd.DataFrame({'Coefficients': coeff})
    else:
        object_to_return = pd.DataFrame({'Taxonome': original_names,
            'Coefficients': coeff})
    if visualize:
...
def roc_auc(y_test, y_score, verbose=False, visualize=False, graph_title=
    'ROC Curve'):
