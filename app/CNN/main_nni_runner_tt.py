...
def main(w=1e-2, df_path_mapping={}, files={}):
    ...
    for c in df.columns[:50]:
        ...
        if str(np.nanmean(all_c_tr)) != 'nan':
            corr_train.append(np.nanmean(all_c_tr))
        if str(np.nanmean(all_c_te)) != 'nan':
            corr_val.append(np.nanmean(all_c_te))
        if str(np.nanmean(all_c_test)) != 'nan':
            corr_test.append(np.nanmean(all_c_test))
        if str(np.nanmean(all_r2_tr)) != 'nan':
            r2_train_all.append(np.nanmean(all_r2_tr))
        if str(np.nanmean(all_r2_te)) != 'nan':
            r2_val_all.append(np.nanmean(all_r2_te))
        if str(np.nanmean(all_r2_test)) != 'nan':
            r2_test_all.append(np.nanmean(all_r2_test))
    ...
    print('###########################################################')
    print('mean train corr: ', np.nanmean(corr_train))
    print('mean validation corr: ', np.nanmean(corr_val))
    print('mean test corr: ', np.nanmean(corr_test))
    print('l2 weight decay is: ', w)
    results[w] = [ np.nanmean(corr_train),np.nanmean(corr_val), np.nanmean(corr_test)]
    return results[w]

if __name__ == '__main__':
    # w=1e-2, d=0
    df_path_mapping = {'vitamine_A': '../../data_microbiome_in_time/vitamineA/ys.csv', 'saliva_gdm':'../../data_microbiome_in_time/saliva/ys.csv',\
        'mucositis': '../../data_microbiome_in_time/mucositis/ys.csv', 'allergy_time_predictions': '../../data_microbiome_in_time/allergy_time_predictions/ys.csv',\
        'gdm_dataset':'../../data_microbiome_in_time/gdm/ys.csv', 'GVHD': '../../data_microbiome_in_time/GVHD/ys.csv', 'pnas': '../../data_microbiome_in_time/pnas/ys.csv', 'diab':'../../data_microbiome_in_time/diab/ys.csv'}
    main(df_path_mapping=df_path_mapping)
