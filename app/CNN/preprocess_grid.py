...
from CNN.distance_learning_func import distance_learning
from sklearn.decomposition import PCA
from sklearn.decomposition import FastICA
from CNN.plot_rho import draw_component_rhos_calculation_figure
...
def preprocess_data(data, dict_params: dict, map_file, visualize_data=False):
    ...
    correlation_removal_threshold = dict_params.get('correlation_threshold',
                                                    None)
    rare_bacteria_threshold = dict_params.get('rare_bacteria_threshold', None)
    var_th_delete = float(dict_params['std_to_delete'])
    pca = dict_params['pca']
    taxonomy_col = 'taxonomy'
    as_data_frame = pd.DataFrame(data.T).apply(pd.to_numeric, errors='ignore'
                                               ).copy()
    ...
    if visualize_data:
        ...
        visualize_preproccess(data_frame_for_vis, indexes_of_non_zeros,
                              'Before Taxonomy group', [321, 322])
    if preform_taxnomy_group != '':
        print('Perform taxonomy grouping...')
        taxonomy_reduced = as_data_frame[taxonomy_col].map(lambda x: x.
                                                           split(';'))
        ...
        if preform_taxnomy_group == 'mean':
            print('mean')
            as_data_frame = as_data_frame.groupby(as_data_frame[taxonomy_col]
                                                  ).mean()
    ...
    if correlation_removal_threshold is not None:
        as_data_frame = dropHighCorr(as_data_frame,
                                     correlation_removal_threshold)
    if visualize_data:
        data_frame_flatten = as_data_frame.values.flatten()
        indexes_of_non_zeros = data_frame_flatten != 0
        visualize_preproccess(as_data_frame, indexes_of_non_zeros,
                              'After-Taxonomy - Before', [323, 324])
...
mean={samples_variance.values.mean()}, std={samples_variance.values.std()}"""
            ...
            plt.savefig(os.path.join(folder, 'samples_variance.svg'),
                        bbox_inches='tight', format='svg')
    ...
    if visualize_data:
        ...
        plt.figure('Preprocess')
        visualize_preproccess(as_data_frame, indexes_of_non_zeros,
                              'After-Taxonomy - After', [325, 326])
        plt.subplots_adjust(hspace=0.5, wspace=0.5)
        plt.savefig(os.path.join(folder, 'preprocess.svg'), bbox_inches=
        'tight', format='svg')
    if visualize_data:
        plt.figure('standard heatmap')
        sns.heatmap(as_data_frame, cmap='Blues', xticklabels=False,
                    yticklabels=False)
        plt.title('Heatmap after standardization and taxonomy group level ' +
                  str(taxnomy_level))
        plt.savefig(os.path.join(folder, 'standard_heatmap.png'))
        corr_method = 'pearson'
        corr_name = 'Pearson'
        plt.figure('correlation heatmap patient')
        sns.heatmap(as_data_frame.T.corr(method=corr_method), cmap='Blues',
                    vmin=-1, vmax=1, xticklabels=False, yticklabels=False)
        plt.title(corr_name + ' correlation patient with taxonomy level ' +
                  str(taxnomy_level))
        plt.savefig(os.path.join(folder, 'correlation_heatmap_patient.png'))
        plt.figure('correlation heatmap bacteria')
        sns.heatmap(as_data_frame.corr(method=corr_method), cmap='Blues',
                    vmin=-1, vmax=1, xticklabels=False, yticklabels=False)
        plt.title(corr_name + ' correlation bacteria with taxonomy level ' +
                  str(taxnomy_level))
    ...
    if preform_taxnomy_group == 'sub PCA':
        as_data_frame, _ = distance_learning(perform_distance=True, level=
        taxnomy_level, preproccessed_data=as_data_frame, mapping_file=
                                             map_file)
        as_data_frame_b_pca = as_data_frame
    if visualize_data:
        draw_component_rhos_calculation_figure(as_data_frame, map_file,
                                               save_folder=folder)
    if pca[0] != 0:
        print('perform ' + pca[1] + ' ...')
        as_data_frame, pca_obj, pca = apply_pca(as_data_frame, n_components
        =pca[0], dim_red_type=pca[1])
    else:
        pca_obj = None
    return as_data_frame, as_data_frame_b_pca, pca_obj, bacteria, pca


def visualize_preproccess(as_data_frame, indexes_of_non_zeros, name,
                          subplot_idx):
...
def row_normalization(as_data_frame):
    as_data_frame = as_data_frame.div(as_data_frame.sum(axis=1), axis=0
                                      ).fillna(0)
    return as_data_frame


def drop_low_var(as_data_frame, threshold):
    drop_list = [col for col in as_data_frame.columns if col != 'taxonomy' and
                 threshold > np.var(as_data_frame[col])]
...
def drop_bacteria(as_data_frame):
    ...
    if len(bacterias_to_dump) != 0:
        print('number of bacterias to dump before intersection: ' + str(len
                                                                        (bacterias_to_dump)))
...
def dropHighCorr(data, threshold):
    corr = data.corr()
    df_not_correlated = ~(corr.mask(np.tril(np.ones([len(corr)] * 2, dtype=
    bool))).abs() > threshold).any()
    un_corr_idx = df_not_correlated.loc[df_not_correlated[df_not_correlated
        .index] == True].index
    df_out = data[un_corr_idx]
    number_of_bacteria_dropped = len(data.columns) - len(df_out.columns)
    print('{} bacteria were dropped due to high correlation with other columns'
          .format(number_of_bacteria_dropped))
    return df_out
...
def drop_rare_bacteria(as_data_frame, threshold):
    ...
    print('{} bacteria with less then {} non-zero value: '.format(len(
        rare_bacteria), threshold))
    return as_data_frame
...
def apply_pca(data, n_components=15, dim_red_type='PCA', visualize=False):
    if n_components == -1:
        ...
        for accu_var, (i, component) in zip(pca.explained_variance_ratio_.
                                                    cumsum(), enumerate(pca.explained_variance_ratio_)):
            if accu_var > 0.7:
                components = i + 1
                break
    else:
    ...
    if dim_red_type == 'PCA':
        ...
        str_to_print = str('Explained variance per component: \n' + '\n'.
                           join([('Component ' + str(i) + ': ' + str(component) +
                                  ', Accumalative variance: ' + str(accu_var)) for accu_var, (i,
                                                                                              component) in
                                 zip(pca.explained_variance_ratio_.cumsum(),
                                     enumerate(pca.explained_variance_ratio_))]))
        str_to_print += str('\nTotal explained variance: ' + str(pca.
                                                                 explained_variance_ratio_.sum()))
        print(str_to_print)
        if visualize:
            plt.figure()
            plt.plot(pca.explained_variance_ratio_.cumsum())
            plt.bar(np.arange(0, components), height=pca.
                    explained_variance_ratio_)
            plt.title(
                f'PCA - Explained variance using {n_components} components: {pca.explained_variance_ratio_.sum()}'
            )
            plt.xlabel('PCA #')
            plt.xticks(list(range(0, components)), list(range(1, components +
                                                              1)))
...
def fill_taxonomy(as_data_frame, tax_col):
    ...
    df_tax = df_tax.fillna(value=np.nan)
    if df_tax.shape[1] == 1:
